...
- **Ask panel** — a grounded AI chat behind a floating launcher (Vercel AI SDK, streaming, one contact-card tool); answers only from the site's content, with per-IP rate limiting and a capped output budget
...
- `components/ask/` — the floating chat launcher + the Ask panel (`<dialog>`)
